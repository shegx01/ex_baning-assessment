...
alias ExBanking.Customer.Supervisor, as: CustomerSupervisor
...
"""
  ...
  def create_user(name) when is_bitstring(name) do
    CustomerSupervisor.start_worker(name)
  end
...
@spec withdraw(user :: String.t(), amount :: number, currency :: String.t()) ::
  ...
  |>Customer.Producer.create_transaction()
   ...
   @spec send(from_user::String.t(), to_user::String.t() , amount::non_neg_integer(), currency::String.t()) ::
           {:error, :wrong_argument} | ExBanking.Customer.Transaction.t()
   def send(from_user, to_user, amount, currency) do
     Customer.Transaction.new(:send, from_user, to_user, amount, currency) |>Customer.Producer.create_transaction()
  end
end
